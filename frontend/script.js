...
function displayItems(items) {
        ...
        itemElement.innerHTML = `
            <div class="card mb-4">
                <div class="card-body">
                    ...
                    <button class="btn btn-primary update-btn" data-id="${item.id}">Update</button>
                </div>
            </div>
        `;
    ...
    document.querySelectorAll('.update-btn').forEach(button => {
        //console.log("Button ID:", button.getAttribute('data-id)); Untek debugging
    ...
    function openUpdateModal(id) {
        ...
        const token = localStorage.getItem('accessToken');
        fetch(`http://127.0.0.1:8000/apia/items/${id}`, {
            header: {
                'Authorization': `Bearer ${token}`
            }
        })
        .then(response => response.json())
        .then(data => {
            //Isi formulir di modal denga data yang ada
